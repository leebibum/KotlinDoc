...
class PrivateObjMember {
    ...
    fun getObjPublic() = object {
        val prop = 0
    }

    private inline fun getObjInline() = object {
        val prop = 0
    }
...
// 슈퍼 타입이 없는 객체의 경우 Any
// 슈퍼 타입이 있는 객체의 경우 해당 객체
// 슈퍼 타입이 다수인 경우 명시적으로 선언된 반환 타입
...
class PublicObjMember {
    ...
    fun getObjWithBaseFunc() = object : BaseForObj() { // BaseForObj
        override fun absFunc() {}
    }
    ...
    fun getObjWith2BaseFunc(): IntfForObj = object : BaseForObj(), IntfForObj { // IntfForObj
        override fun absFunc() {}
        override fun intfFunc() {}
    }
}
...
class CompanionObj {
    ...
    val prop = privateProp // 클래스 멤버는 컴패니언 오브젝의 private 멤버도 참조 가능
}

// 컴패니언 오브젝트 참조
fun companionObjReference() {
    CompanionObj.CompanionObjName.prop // 객체 생성 없이 참조, 이름 생략 가능

    val companionObj = CompanionObj // 클래스 이름으로 컴패니언 오브젝트 참조 가능
}

// static과 동일하게 보이지만 런타임 시 실제 인스턴의 멤버로 취급

// 컴패니언 오브젝트도 인터페이스 구현 가능
class CompanionObjWithIntf {
    companion object : IntfForObj {
        override fun intfFunc() {}
    }
}
// @JvmStatic 어노테이션으로 JVM에서 실제 static으로 사용 가능
